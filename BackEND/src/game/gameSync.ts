...
export class GameSyncManager {
  ...
  // Broadcast leaderboard update to all in room
  ...
  handleQuizStarted(socket: Socket, playerId: string, quizCategory: string): void {
    ...
    console.log(`⏸️ Game paused in room ${room.id} - ${playerId} started quiz: ${quizCategory}`);
  }
  ...
  startTimeProgression(roomId: string, monthDurationMs: number): NodeJS.Timeout | null {
    ...
    const interval = setInterval(() => {
      ...
      console.log(`⏰ Room ${roomId} - Year ${newYear}, Month ${newMonth}`);
    }, monthDurationMs);
